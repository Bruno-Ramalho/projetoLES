export const ORDER_CREATE_REQUEST = 'ORDER_CREATE_REQUEST';
export const ORDER_CREATE_SUCCESS = 'ORDER_CREATE_SUCCESS';
export const ORDER_CREATE_FAIL = 'ORDER_CREATE_FAIL';
export const ORDER_CREATE_RESET = 'ORDER_CREATE_RESET';

export const ORDER_DETAILS_REQUEST = 'ORDER_DETAILS_REQUEST';
export const ORDER_DETAILS_SUCCESS = 'ORDER_DETAILS_SUCCESS';
export const ORDER_DETAILS_FAIL = 'ORDER_DETAILS_FAIL';

export const ORDER_USER_LIST_REQUEST = 'ORDER_USER_LIST_REQUEST';
export const ORDER_USER_LIST_SUCCESS = 'ORDER_USER_LIST_SUCCESS';
export const ORDER_USER_LIST_FAIL = 'ORDER_USER_LIST_FAIL';

export const ORDER_CHANGE_STATUS_REQUEST = 'ORDER_CHANGE_STATUS_REQUEST';
export const ORDER_CHANGE_STATUS_SUCCESS = 'ORDER_CHANGE_STATUS_SUCCESS';
export const ORDER_CHANGE_STATUS_FAIL = 'ORDER_CHANGE_STATUS_FAIL';

export const ORDER_LIST_REQUEST = 'ORDER_LIST_REQUEST';
export const ORDER_LIST_SUCCESS = 'ORDER_LIST_SUCCESS';
export const ORDER_LIST_FAIL = 'ORDER_LIST_FAIL';

export const ORDER_DETAILS_CREATE_REQUEST = 'ORDER_DETAILS_CREATE_REQUEST';
export const ORDER_DETAILS_CREATE_SUCCESS = 'ORDER_DETAILS_CREATE_SUCCESS';
export const ORDER_DETAILS_CREATE_FAIL = 'ORDER_DETAILS_CREATE_FAIL';

export const ORDER_GRAPH_REQUEST = 'ORDER_GRAPH_REQUEST';
export const ORDER_GRAPH_SUCCESS = 'ORDER_GRAPH_SUCCESS';
export const ORDER_GRAPH_FAIL = 'ORDER_GRAPH_FAIL';